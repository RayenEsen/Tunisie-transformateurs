...
export class AddComponentComponent implements OnInit {
  ...
  validateForm(currentTransformateur: Transformateur): string {
    if (currentTransformateur.etat === '' || currentTransformateur.etat === undefined) {
        return 'Mission';
    }
    if (currentTransformateur.client === '' || currentTransformateur.client === undefined) {
        return 'Client';
    }
    if (currentTransformateur.power === '' || currentTransformateur.power === undefined) {
        return 'Puissance';
    }
    if (currentTransformateur.mtu1 === 0 || currentTransformateur.mtu1 === undefined) {
        return 'MTU1';
    }
    if (currentTransformateur.mtu2 === 0 || currentTransformateur.mtu2 === undefined) {
        return 'MTU2';
    }
    if (currentTransformateur.btu2 === 0 || currentTransformateur.btu2 === undefined) {
        return 'BTU2';
    }
    if (currentTransformateur.prises === '' || currentTransformateur.prises === undefined) {
        return 'Prises';
    }
    if (currentTransformateur.libelle === '' || currentTransformateur.libelle === undefined) {
        return 'Libellé';
    }
    if (currentTransformateur.type === '' || currentTransformateur.type === undefined) {
        return 'Type';
    }
    if (currentTransformateur.couplage === '' || currentTransformateur.couplage === undefined) {
      return 'Couplage';
    }

    if (currentTransformateur.dateCloture === null || currentTransformateur.dateCloture === undefined) {
      return 'Date Cloture';
    }
    if (currentTransformateur.dateprevue === null || currentTransformateur.dateCloture === undefined) {
      return 'Date revue';
    }

    return ''; // Form is valid if none of the conditions are met
}
  ...
  submitForm() {
    ...
    if(this.transformateurAjouter.quantite2<=0)
    {
      this.messageService.add({ severity: 'error', summary: 'Erreur', detail: "Une quantité minimale de 1 unité est requise." });
      return;
    }
    if (this.validateForm(this.transformateurAjouter)!=='') {
      this.messageService.add({ severity: 'error', summary: 'Erreur', detail:  this.validateForm(this.transformateurAjouter)+' est obligatoire' });
      return;
    }

    for (let i = this.nb+1; i < this.transformateurAjouter.quantite2+this.nb+1; i++) {

      try {

        let currentTransformateur = Object.assign({}, this.transformateurAjouter); // Create a copy of this.transformateurAjouter
        currentTransformateur.numero = i;
            ...
            concatMap(() => {
              ...
              for (let i = 1; i <= 9; i++) {
                const BobinageAjouter: Bobinage = {
                  idBobinage: 0,
                  numero: currentTransformateur.numero,
                  nom: BobinageNames[i] || ''
                };
